...
interface MainCategoriasProps {
  categoriaActiva: CategoriaLote | null;
  onSeleccionarCategoria: (categoria: CategoriaLote | null) => void;
}

export default function MainCategorias({ categoriaActiva, onSeleccionarCategoria }: MainCategoriasProps) {
  // Toggle: si ya está seleccionada, al volver a tocarla se desactiva el filtro
  const manejarClick = (categoria: CategoriaLote) => {
    onSeleccionarCategoria(categoriaActiva === categoria ? null : categoria);
  };
  ...
  return (
    <section className="px-10 mt-32 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <button
          onClick={() => manejarClick('Panaderia')}
          className={`md:col-span-2 bg-[#fef5d4] rounded-3xl p-8 relative overflow-hidden group cursor-pointer hover:shadow-xl transition-all duration-300 text-left ${categoriaActiva === 'Panaderia' ? 'border-2 border-[#1A103C] shadow-xl' : 'border border-yellow-100'}`}
        >
        ...
        <button
          onClick={() => manejarClick('Buffet')}
          className={`bg-[#d4eeff] rounded-3xl p-8 cursor-pointer hover:shadow-xl transition-all duration-300 text-left group ${categoriaActiva === 'Buffet' ? 'border-2 border-[#1A103C] shadow-xl' : 'border border-blue-100'}`}
        >
        ...
        <button
          onClick={() => manejarClick('Sushi')}
          className={`bg-[#e9dfff] rounded-3xl p-8 cursor-pointer hover:shadow-xl transition-all duration-300 text-left group ${categoriaActiva === 'Sushi' ? 'border-2 border-[#1A103C] shadow-xl' : 'border border-purple-100'}`}
        >
          <div className="text-4xl mb-6 transform group-hover:scale-110 transition-transform">🍣</div>
